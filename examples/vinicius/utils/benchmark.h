...
typedef struct benchmark_send_control_t {
    ...
    mutex_t num_packets_count_mutex;
...
static void increment_packets_count() {
    while(!mutex_try_lock(&benchmark_tx_control->num_packets_count_mutex))
        ;
    // Safe region
    benchmark_tx_control->num_packets_sended++;
    // Safe region
    mutex_unlock(&benchmark_tx_control->num_packets_count_mutex);
}
...
static void registers_heap_var(void *pointer) {
    FILE *file =
...
static void create_benchmark_registry() {
    ...
    mutex_unlock(&benchmark_tx_control->num_completed_mtx);
    benchmark_tx_control->num_completed = 3;
    //=======================================================
